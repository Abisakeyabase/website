    ...
    const CourseSection = ({ title, courses }) => (
        <div>
            <div className='flex justify-center mt-8 sm:mt-12'>
                <h1 data-aos="fade-up" data-aos-duration="500" className='font-bold text-xl sm:text-2xl md:text-3xl capitalize'>{title}</h1>
            </div>
            <div data-aos="fade-up" data-aos-duration="500"
                data-aos-anchor-placement="top-bottom">
                <div className='flex flex-wrap mt-8 sm:mt-15 px-2 sm:px-5 justify-center gap-3 sm:gap-5'>
                    {courses.map((course, index) => (
                        <div className=' xl:w-[22%] lg:w-[28%] sm:w-[45%] md:w-[48%] flex flex-col' key={course.id} data-aos-duration="500" data-aos={index % 2 === 0 ? "fade-right" : "fade-left"}>
                                ...
                                <div className='mt-2 mb-4 flex justify-between pb-4 sm:pb-5 px-4 sm:px-8 py-1 md:p-1'>
                                    {course.available == "yes" ? (
                                        <>
                                            ...
                                            <button className='courses-btn text-white capitalize font-bold px-2 sm:px-3 py-1 lg:p-1 rounded-lg hover:cursor-pointer sm:text-base' onClick={() => formsetSelectedCourse(course)}>download brochure</button>
                                            <button className='courses-btn text-white capitalize font-bold px-2 sm:px-3 py-1 lg:p-1 rounded-lg hover:cursor-pointer sm:text-base' onClick={() => navigate(`/course-details/${course.id}`)}>know more</button>
...
<section className='sec sm:mt-15'>
            <div className=''>
                <h1 className='flex justify-center font-bold text-2xl sm:text-3xl md:text-4xl lg:text-5xl'>
            ...
            <div data-aos="fade-up" className='mt-6 md:mt-10 px-4'>
                <div className='gradient-border w-full md:w-[90%] lg:w-[70%] xl:w-[60%] m-auto py-6 md:py-10 rounded-lg'>
                    <h1 className='w-fit m-auto text-xl md:text-2xl lg:text-3xl font-bold capitalize text-center'>VTS <span className='abt-title'>certificate</span></h1>

                    <div className='flex justify-center items-center mt-4 md:mt-8 px-2'>
                        <img
                    ...
                    <h1 className='w-full text-center mt-2 md:mt-3 m-auto uppercase text-lg md:text-xl lg:text-2xl font-bold'>
                        VETRI TECHNOLOGY SOLUTIONS
                    </h1>

                    <h1 className='w-full text-center mt-2 md:mt-3 m-auto uppercase text-md md:text-lg lg:text-xl font-bold'>
                        COMPLETION CERTIFICATE
                    </h1>

                    <hr className='w-[90%] m-auto mt-2 md:mt-3' />

                    <p className='w-full md:w-[90%] m-auto mt-2 md:mt-3 text-center font-medium text-sm md:text-base lg:text-lg px-2'>
                        VTS provides you a Training Completion Certificate with an Online Verification & QR Code Verification which can be downloaded anytime with the credentials provided to the Student. It is an ISO 9001:2015 Certificate which is valid everywhere. Once you complete the IT Training Successfully with VTS, you will receive an IT Job Offer or Guaranteed Internship with Stipend from our own IT Startup VETRI IT Systems Private Limited, Located at Surandai - Tenkasi District.
                    </p>
                </div>
            </div>
        </section>
